...
export function KwaiCut() {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <Metric title="Permitida para uso" value={countStatus('campaign_allowed')} good />
...
const STATUS_LABELS: Record<string, string> = { campaign_allowed: 'Permitida para uso', review_required: 'aguardando confirmação', discovered: 'descoberta', authorized: 'autorizada', licensed: 'licenciada', approved: 'aprovada', blocked: 'bloqueada' };
...
function ProspectReview({ prospects, value, setValue, add, busy, reload, setError }: { prospects: Prospect[]; value: string; setValue: (value: string) => void; add: () => void; busy: boolean; reload: () => Promise<void>; setError: (value: string | null) => void }) {
  ...
  const selectFiltered = () => setPicked(new Set(filtered.filter((i) => i.review_status === 'review_required' || i.review_status === 'discovered').map((i) => i.prospect_id)));
  const clearPicked = () => setPicked(new Set());
  const bulkAction = async (action: 'permit' | 'block') => {
    if (bulkBusy || !picked.size) return;
    if (action === 'permit' && !confirmed) { setError('Marque a confirmação de permissão antes de liberar as fontes.'); return; }
    setBulkBusy(true); setError(null);
    const items = prospects.filter((i) => picked.has(i.prospect_id)).map((i) => ({ prospect_id: i.prospect_id, source_url: i.source_url }));
    const response = await fetch('/api/kwai/prospects/bulk', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action, confirmed, items, blocked_reason: 'triagem em lote' }) });
    if (response.ok) { const r = await response.json(); setError(null); clearPicked(); setConfirmed(false); await reload();
      setError(`${action === 'permit' ? 'Permitidas' : 'Bloqueadas'}: ${r.done}. Falhas: ${r.failed}. As permitidas entram no produtor no próximo ciclo.`); }
    else setError('A operação em lote foi recusada.');
    setBulkBusy(false);
  };
