...
class MigrationTests(unittest.TestCase):
    def test_upgrade_creates_only_shop_live_tables(self):
        ...
        with tempfile.TemporaryDirectory(dir=Path(__file__).parent) as temp:
            ...
            result = subprocess.run([str(root / ".venv" / "Scripts" / "python.exe"), "-m", "alembic", "upgrade", "head"], cwd=root, env=env, capture_output=True, text=True)
            self.assertEqual(result.returncode, 0, result.stderr)
            engine = create_engine(f"sqlite:///{database.as_posix()}")
            tables = set(inspect(engine).get_table_names())
            engine.dispose()
            self.assertEqual(tables, {"alembic_version", "shop_live_products", "shop_live_sessions", "shop_live_audit_events"})
