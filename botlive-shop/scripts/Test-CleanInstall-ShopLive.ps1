...
try {
  ...
  for($Attempt=0;$Attempt -lt 30;$Attempt++){try{$Health=Invoke-RestMethod 'http://127.0.0.1:8765/shop-live/v1/health' -Headers $Headers;if($Health.ok){break}}catch{};Start-Sleep -Milliseconds 500};if($Attempt -ge 30){throw 'Autenticacao/health falhou'}
  ...
  $UploadedRaw=& curl.exe -sS --fail -H "X-Shop-Live-Token: $Token" -F "file=@$Wave;type=audio/wav" -F "authorized=true" -F "authorization_source=Teste limpo Windows" 'http://127.0.0.1:8765/shop-live/v1/media/upload';if($LASTEXITCODE -ne 0){throw 'Upload autorizado falhou'};$Uploaded=$UploadedRaw|ConvertFrom-Json;$Library=Invoke-RestMethod 'http://127.0.0.1:8765/shop-live/v1/library' -Headers $Headers;if($Library.media.id -notcontains $Uploaded.id){throw 'Upload não apareceu na biblioteca'}
