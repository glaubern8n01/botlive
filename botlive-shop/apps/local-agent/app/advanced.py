...
def duplicate_media(item_id:str,db:Session=Depends(get_db)):
    ...
    original=safe_path(source.stored_name);new_name=f"{uuid4().hex}{original.suffix.lower()}";target=safe_path(new_name);shutil.copyfile(original,target)
...
def backup(db:Session=Depends(get_db)):
    backup_root=Path(os.getenv("SHOP_LIVE_BACKUP_ROOT","./data/backups")).resolve();backup_root.mkdir(parents=True,exist_ok=True);target=backup_root/f"shop-live-{datetime.now().strftime('%Y%m%d-%H%M%S')}.zip"
    database_url=os.getenv("SHOP_LIVE_DATABASE_URL","sqlite:///./data/shop-live.db");database=Path(database_url.removeprefix("sqlite:///")).resolve()
    with zipfile.ZipFile(target,"w",zipfile.ZIP_DEFLATED) as archive:
        if database.is_file(): archive.write(database,"database.sqlite")
        for path in storage_root().iterdir():
